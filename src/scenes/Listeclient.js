...
function Listeclient ({ navigation }) {

  const [item,setItem]= useState([])
  useEffect(()=>{
      axios.get('http:192.168.1.12:4002/client/fiche')
      .then(response=>
    ...
    return (
        ...
        <Headline style={{color:"#ff80f7" ,fontWeight:'bold',fontSize:30}}>  Afficher les Clients :</Headline>
        ...
        <ListItem itemHeader first Divider  >
              ...
              <Left>
                ...
                <Text style={{fontSize:24 }} > {item.nomcli} {item.prenomcli} </Text>
                <Text style={{fontSize:20 }}> {item.villecli} {item.adressecli}</Text>
                </Body>
                
              </Left>
              <Button transparent onPress={() => navigation.navigate('Fiche client',{
                cinid:item.cincli,
                nom:item.nomcli ,
